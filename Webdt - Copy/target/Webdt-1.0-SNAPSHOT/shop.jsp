        ...
        <div class="row">
            <div class="col-md-12">
                <div class="product-pagination text-center">
                    <nav>
                        <ul class="pagination">
                            <li><a href="ProductListController?page=<%=pagePrevious%>&id=${id}">Previous</a></li>
                            <c:forEach begin="1" end="${endPage}" var="i">
                                <c:choose>
                                    <c:when test="${pageIsSelected == i}">
                                        <li>
                                            <a href="ProductListController?page=${i}&id=${id}"
                                               class="selected">${i}</a>
                                                <%--                                            <% pageNow = ${i} %>--%>
                                        </li>
                                    </c:when>
                                    <c:otherwise>
                                        <li>
                                            <a href="ProductListController?page=${i}&id=${id}">${i}</a>
                                        </li>
                                    </c:otherwise>

                                </c:choose>
                            </c:forEach>
                            <li><a href="ProductListController?page=<%=pageNext%>&id=${id}">Next</a></li>
                        </ul>
                    </nav>
                </div>
            </div>
